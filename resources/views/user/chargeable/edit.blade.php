@extends('layouts.app')
@section('title','Chargeable Request Form')
@section('content')
    ...
    {{-- CONTENT --}}
        <div class="w-screen h-screen pt-14">
            <form action="{{ route('chargeable.update') }}" method="POST" enctype="multipart/form-data" id="requestForm" class="relative h-full">
                @csrf
                <!-- body -->
                    ...
                    <div id="tab-container" class="w-[400%] h-full transition-all duration-1000 ease-in flex -translate-x-0">
                        <div id="tab1" class="w-1/4 h-full mr-4 space-y-4 overflow-x-hidden overflow-y-auto">
                            ...
                            <div class="w-full">
                                ...
                                <textarea style="resize: none;" id='requestor_remarks' name='requestor_remarks' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 h-32" autocomplete="off">{{ $c_request->requestor_remarks }}</textarea>
                            </div>
