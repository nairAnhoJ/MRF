...
@section('content')
            ...
            <div class="w-5/6 h-full bg-white rounded-lg">
                ...
                <form wire:submit='update' class="relative h-full bg-white rounded-lg shadow">
                    ...
                    <div class="flex items-start justify-between p-4 border-b rounded-t">
                        ...
                        <button type="button" wire:click='addSelectedParts' class="inline-flex items-center justify-center w-8 h-8 ml-auto text-sm text-gray-400 bg-transparent rounded-lg hover:bg-gray-200 hover:text-gray-900">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                            </svg>
                            <span class="sr-only">Close modal</span>
                    ...
                    <div class="p-4 h-[calc(100%-140px)] overflow-hidden">
                        <div class="flex items-center justify-between h-11">
                            ...
                            <div class="relative w-1/2 h-full mb-2">
                                ...
                                <input type="search" wire:model.live.debounce.200ms='partSearch' class="block w-full h-full pl-10 pr-4 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search Part Number or Part Name ..." required autocomplete="off">
                            </div>
                        </div>
                    ...
                    <div class="flex items-center p-4 space-x-2 border-t border-gray-200 rounded-b">
                        ...
                        <button type="button" wire:click='addSelectedParts' class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium w-24 py-2.5 hover:text-gray-900 focus:z-10">CLOSE</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    {{-- CONTENT --}}
        <div class="w-screen h-screen pt-14">
            <form action="{{ route('nchargeable.store') }}" method="POST" class="relative h-full">
                <!-- body -->
                <div class="p-4 h-[calc(100%-75px)] overflow-hidden">
                    <div class="w-[400%] h-full transition-all duration-1000 ease-in flex -translate-x-0">
                    {{-- @if($tab == 1)
                        -translate-x-0
                    @elseif($tab == 2)
                        -translate-x-[calc(25%+16px)]
                    @elseif($tab == 3)
                    ...
                    @endif"> --}}
                        <div id="tab1" class="w-1/4 h-full mr-4 space-y-4 overflow-x-hidden overflow-y-auto">
                            ...
                            <div class="flex gap-x-4">
                                ...
                                <div class="w-full">
                                    ...
                                    <input type="date" name='date_needed' id="date_needed" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    @error('date_needed')
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                                    ...
                                    <div class="absolute z-50 hidden w-full p-3 mt-1 border border-gray-300 rounded-md bg-gray-50 content">
                                        ...
                                        <ul class="mt-2 overflow-y-auto listOption options max-h-64">
                                            @foreach ($customers as $customer)
                                                <li date-name="{{ $customer->name }}" date-address="{{ $customer->address }}" date-area="{{ $customer->area }}" class="flex items-center pl-3 leading-9 rounded-md cursor-pointer h-9 hover:bg-gray-200 customerSelected"><span>{{ $customer->name }}</span><span class="ml-2">📌{{ $customer->address }}</span></li>
                                            @endforeach
                                        </ul>
                                    </div>
                            ...
                            <div class="flex gap-x-4">
                                <div class="w-full">
                                    ...
                                    <input type="text" id="customer_address" name='customer_address' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                                ...
                                <div class=" w-96">
                                    ...
                                    <input type="text" id="customer_area" name='customer_area' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly>
                                    @error('customer_area')
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="flex gap-x-4">
                                <div class="w-full">
                                    ...
                                    <select id="brand" name='brand' wire:change='brandSelected' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="relative w-full wrapper">
                                        <div class="flex items-center justify-between p-2.5 border border-gray-300 rounded-md bg-gray-50  h-[42px] {{ ($brand != '') ? 'select-btn cursor-pointer' : ' opacity-60' }}">
                                            <span>{{ ($model != '') ? $model : 'Select Model' }}</span>
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-full text-gray-700 transition-transform duration-300" viewBox="0 -960 960 960" fill='currentColor'><path d="M480-322 216-586l67-67 197 198 197-197 67 67-264 263Z"/></svg>
                                        </div>
                                        <div class="absolute z-50 hidden w-full p-3 mt-1 border border-gray-300 rounded-md bg-gray-50 content">
                                            <div class="relative search">
                                            ...
                                            <ul class="mt-2 overflow-y-auto listOption options max-h-64">
                                                @foreach ($models as $modelrow)
                                                    <li wire:click="modelSelected('{{ $modelrow->name }}')" class="flex items-center pl-3 leading-9 rounded-md cursor-pointer h-9 hover:bg-gray-200">{{ $modelrow->name }}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                                ...
                                <div class="w-full">
                                    ...
                                    <input type="text" id="serial_number" name='serial_number' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" autocomplete="off">
                                    @error('serial_number')
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="w-full">
                                    ...
                                    <div class="flex gap-x-2">
                                        <input type="text" name='fleet_number' wire:keydown='fnInput' id="fleet_number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" autocomplete="off">
                                        <button disabled  class="disabled:pointer-events-none disabled:opacity-60 h-[42px] bg-gray-100 border rounded-lg border-gray-300 aspect-square p-[6px] text-gray-700">
                            ...
                            <div class="flex gap-x-4">
                                <div class="w-full">
                                    ...
                                    <input type="text" id="fsrr_number" name='fsrr_number' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" autocomplete="off">
                                    @error('fsrr_number')
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="w-full">
                                    ...
                                    <div class="flex gap-x-2">
                                        <input type="file" name="fsrrFile" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" accept=".jpeg, .jpg, .png, .pdf">
                                        <button disabled type="button" wire:click='viewFsrr' class="disabled:pointer-events-none disabled:opacity-60 h-[42px] bg-gray-100 border rounded-lg border-gray-300 aspect-square p-[6px] text-gray-700">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill=currentColor>
                            ...
                            <div class="w-full">
                                ...
                                <textarea style="resize: none;" name='requestor_remarks' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 h-20" autocomplete="off"></textarea>
                                {{-- <input type="text" id="requestor_remarks" name='requestor_remarks' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" autocomplete="off"> --}}
                            </div>
                        </div>
                        <div id="tab2" class="w-1/4 h-full mr-4 overflow-x-hidden overflow-y-auto">
                            <div class="sticky top-0 flex items-center justify-between pb-4 bg-white">
                                ...
                                <button wire:click='addParts' type="button" class="text-blue-500 hover:text-blue-600">
                            ...
                            {{-- <input type="text" id="part_search" wire:model.live='hiddenInputValue'> --}}
                            <div class="w-full">
                                <div class="w-full ">
                ...
                <div class="flex items-center justify-between p-4 space-x-2 border-t border-gray-200 rounded-b">
                    <button type="button" wire:click='cancel' class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium py-2.5 hover:text-gray-900 focus:z-10 ml-2 w-24">CANCEL</button>
                    <div>
                        {{-- <button wire:click="store" type="submit" class="{{ ($tab < 3) ? 'hidden' : '' }} text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm py-2.5 text-center w-24">SUBMIT</button> --}}
                        ...
                        {{-- <button wire:click="backBtn" type="button" class="{{ ($tab > 1) ? '' : 'hidden' }} text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm py-2.5 text-center w-24">BACK</button> --}}
                    </div>
                </div>
            </form>
        </div>
