    ...
    <div class="w-full h-full p-4">
        ...
        {{-- CONTROLS --}}
            <div class="flex items-center justify-between w-full h-12">
                <div class="h-full">
                    {{-- ADD USER --}}
                        <a href="{{ route('approvers.add') }}" class="flex items-center h-full px-8 text-lg font-bold text-white bg-blue-500 rounded-lg loading">ADD</a>
                    {{-- ADD USER --}}
                </div>
                    {{-- SEARCH --}}
                        <form action="{{ route('approvers.index') }}" method="GET" class="w-96">
                            @csrf
                    ...
                    <table class="w-full rounded-lg">
                        ...
                        <tbody>
                            @foreach ($approvers as $approver)
                                <tr class="text-center bg-gray-100 cursor-pointer even:bg-gray-200 hover:bg-gray-400">
                                    <td>{{ $approver->for }}</td>
                                    <td>{{ $approver->condition }}</td>
                                    <td>{{ $approver->user->name }}</td>
                                    <td class="font-semibold">
                                        <a href="{{ route('approvers.edit', ['id' => $approver->id]) }}" class="text-blue-500 hover:underline">EDIT</a> | 
                                        <button type="button" data-key="{{ $approver->key }}" class="text-red-500 hover:underline deleteButton">DELETE</button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
